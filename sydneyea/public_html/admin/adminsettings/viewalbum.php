<?php include('header.php'); 
	...
	if($flag == "active")
	{
		 $Update_User	=	"UPDATE ".ALBUM." SET `varstatus`='Active' WHERE `aid`='$id'";
	...
	elseif($flag == "deactive")
	{
		...
		$Update_User	=	"UPDATE ".ALBUM." SET `varstatus`='DeActive' WHERE `aid`='$id'";
		...
		elseif($flag == "delete")
	{
		$Delete_User	=	"DELETE FROM ".ALBUM." WHERE `aid`='$id'";
		$Result_User	=	mysql_query($Delete_User) or die(mysql_error());
		$rflag	=	"success";
		$rType	=	"delete";
	}

  	$pagqry="SELECT * FROM ".ALBUM." WHERE `albumname` LIKE '$SortList%'";
...
//$limit = 2; 
	...
	$limit  = $pager->limit; 
	 $page   = $pager->page; 	
 	 ...
 	 $limqry		= "SELECT * FROM ".ALBUM." WHERE `albumname` LIKE '$SortList%'  order by aid DESC  limit $offset,$limit ";
...
<div id="page-wrapper">
		<div id="main-wrapper">
			<div id="main-content">
			<div class="title">
				<a name="viewcat"></a>	<h3>View Albums</h3>
				</div>
 ...
 <td>&nbsp;</td>
  ...
  <tr>
           <td colspan="8" align="center" style="padding-left:55px;">
					   <?php
						echo "<a class=\"leftlinks\" href=\"viewalbum.php?sortby=all\">All</a>";?>
									  &nbsp;&nbsp;&nbsp;
									  <?php 		for($ASCII = 65; $ASCII <= 90 ; $ASCII++)
						{				
						  echo "<a class=\"leftlinks\" href=\"viewalbum.php?sortby=".chr($ASCII)."\">".chr($ASCII)."</a>&nbsp;" ."&nbsp;";
						}	
					 ?>		 	</td>
  </tr>
  ...
  { 
	  if($rType == "active")
	  	$rDisp	=	"Album Status Successfully Activated";
	  elseif($rType == "deactive")
	  	$rDisp	=	"Album Status Successfully De-Activated";
	  elseif($rType == "delete")
	  	$rDisp	=	"Album Deleted Successfully"; 
	 elseif($rType == "add")
		$rDisp	=	"Album Added Successfully"; 
	 elseif($rType == "edit")
		$rDisp	=	"Album Edited Successfully"; 
}
	echo $rDisp;
?></span>
			</div><?php }?>
				...
				<div class="hastable">
					<form name="myform" class="" method="post" action="">
						...
						<tr>
						    ...
						    <th width="274" align="left">Album Name</th> 
					...
					?>
						<tr>
						    ...
						    <td style="padding-left:110px;"><?php echo ucfirst($fetch_cat['albumname']);?></td> 
							
							    <td align="center" style="padding-left:55px;"><?php if($fetch_cat['varstatus'] == "Active") { ?>
                                <a href="viewalbum.php?flag=deactive&amp;cid=<?php echo $fetch_cat['aid'];?>&page=<?php echo $_GET['page'];?>"  class="btn_no_text btn ui-state-default ui-corner-all tooltip" title="De-Activate">
								<span class="ui-icon ui-icon-unlocked"></span></a>
                                <?php } else { ?>
                                <a href="viewalbum.php?flag=active&amp;cid=<?php echo $fetch_cat['aid'];?>&page=<?php echo $_GET['page'];?>"  class="btn_no_text btn ui-state-default ui-corner-all tooltip" title="Activate">
								<span class="ui-icon ui-icon-locked"></span></a><?php } ?></td> 
						 
						 
						    <td align="center" style="padding-left:28px;"><a href="addalbum.php?flag=edit&amp;cid=<?php echo $fetch_cat['aid'];?>&page=<?php echo $_GET['page'];?>"class="btn_no_text btn ui-state-default ui-corner-all tooltip" title="Edit This album">
							<span class="ui-icon ui-icon-pencil"></span>
							</a>
							
							</td> 
						    <td align="center" style="padding-left:40px;"><a href="viewalbum.php?flag=delete&amp;cid=<?php echo $fetch_cat['aid'];?>&page=<?php echo $_GET['page'];?>" class="btn_no_text btn ui-state-default ui-corner-all tooltip" title="Delete"><span class="ui-icon ui-icon-circle-close" onClick="return calldel('Do You Want To Delete This song?');"></span></a></td> 
					      </tr> 
						  <?php 
          ...
          <tr>
            <td width="97%" align="right" class="test"><?php 					
					if($limit < $total)
					{
							...
							{       
								echo "<a href=\"viewalbum.php?sortby=".$SortList."&page=" . ($page - 1) . "\" class='linksnew'>Prev</a>"; 
							}
							...
							{ 
								...
								{	 
									echo "<a href=\"viewalbum.php?sortby=".$SortList."&page=$i\" class='linksnew'> $i</a>"."</font>"; 
								}
							} 
							...
							{     
								echo "<a href=\"viewalbum.php?sortby=".$SortList."&page=" . ($page + 1) . "\" class='linksnew'>Next</a>";
							}
					}		
