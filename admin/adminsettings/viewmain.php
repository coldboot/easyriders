<?php include('header.php'); 
	...
	if($flag == "active")
	{
		 $Update_User	=	"UPDATE tbl_gigband_main SET `status`='Active' WHERE `mid`='$id'";
	...
	elseif($flag == "deactive")
	{
		...
		$Update_User	=	"UPDATE  tbl_gigband_main  SET `status`='DeActive' WHERE `mid`='$id'";
		...
		elseif($flag == "delete")
	{
		$Delete_User	=	"DELETE  FROM tbl_gigband_main  WHERE `mid`='$id'";
		$Result_User	=	mysql_query($Delete_User) or die(mysql_error());
		$rflag	=	"success";
		$rType	=	"delete";
	}

  	$pagqry="SELECT * FROM  tbl_gigband_main  WHERE `maintitle` LIKE '$SortList%'";
...
//$limit = 2; 
	...
	$limit  = $pager->limit; 
	 $page   = $pager->page; 	
 	 ...
 	 $limqry		= "SELECT * FROM  tbl_gigband_main  WHERE `maintitle` LIKE '$SortList%'  order by mid DESC  limit $offset,$limit ";
 ...
 <td>&nbsp;</td>
  ...
  <tr>
           <td colspan="8" align="center" style="padding-left:55px;">
					   <?php
						echo "<a class=\"leftlinks\" href=\"viewmain.php?sortby=all\">All</a>";?>
									  &nbsp;&nbsp;&nbsp;
									  <?php 		for($ASCII = 65; $ASCII <= 90 ; $ASCII++)
						{				
						  echo "<a class=\"leftlinks\" href=\"viewmain.php?sortby=".chr($ASCII)."\">".chr($ASCII)."</a>&nbsp;" ."&nbsp;";
						}	
					 ?>		 	</td>
  </tr>
					...
					?>
						<tr>
						    ...
						    <td style="padding-left:110px;"><?php echo ucfirst($fetch_cat['maintitle']);?></td> 
                                ...
                                <a href="viewmain.php?flag=deactive&amp;cid=<?php echo $fetch_cat['mid'];?>"  class="btn_no_text btn ui-state-default ui-corner-all tooltip" title="De-Activate">
								<span class="ui-icon ui-icon-unlocked"></span></a>
                                <?php } else { ?>
                                <a href="viewmain.php?flag=active&amp;cid=<?php echo $fetch_cat['mid'];?>"  class="btn_no_text btn ui-state-default ui-corner-all tooltip" title="Activate">
								<span class="ui-icon ui-icon-locked"></span></a><?php } ?></td> 
						 
						 
						    <td align="center" style="padding-left:28px;"><a href="addmain.php?flag=edit&amp;cid=<?php echo $fetch_cat['mid'];?>"class="btn_no_text btn ui-state-default ui-corner-all tooltip" title="Edit This Title">
							<span class="ui-icon ui-icon-pencil"></span>
							</a>
							
							</td> 
						    <td align="center" style="padding-left:40px;"><a href="viewmain.php?flag=delete&amp;cid=<?php echo $fetch_cat['mid'];?>" class="btn_no_text btn ui-state-default ui-corner-all tooltip" title="Delete"><span class="ui-icon ui-icon-circle-close" onClick="return calldel('Do You Want To Delete This Title?');"></span></a></td> 
					      </tr> 
						  <?php 
          ...
          <tr>
            <td width="97%" align="right" class="test"><?php 					
					if($limit < $total)
					{
							...
							{       
								echo "<a href=\"viewmain.php?sortby=".$SortList."&page=" . ($page - 1) . "\" class='linksnew'>Prev</a>"; 
							}
							...
							{ 
								...
								{	 
									echo "<a href=\"viewmain.php?sortby=".$SortList."&page=$i\" class='linksnew'> $i</a>"."</font>"; 
								}
							} 
							...
							{     
								echo "<a href=\"viewmian.php?sortby=".$SortList."&page=" . ($page + 1) . "\" class='linksnew'>Next</a>";
							}
					}		
